...
// TODO Colocar campo para selecionar a data
...
function tpSolicitacaoTT() {
  movelData.forEach((e) => {
    if (
      e.TP_SOLICITACAO != "Alta" &&
      !e.TP_SOLICITACAO.startsWith("Troca") &&
      e.TP_SOLICITACAO != "Aparelho"
    ) {
      tt.push(e);
    }
...
function tpSolicitacaoTroca0() {
  movelData.forEach((e) => {
    if (
      e.TP_SOLICITACAO.includes("Troca") ||
      e.TP_SOLICITACAO.startsWith("Troca")
    ) {
      Troca0.push(e);
    }
...
function migracao() {
  movelData.forEach((e) => {
    if (
      (e.TP_SOLICITACAO.includes("Troca") ||
        e.TP_SOLICITACAO.startsWith("Troca")) &&
      e.DS_SERVICO_INTERNET.startsWith("SM") &&
      e.DS_PLANO_EXPIRADO === 0
    ) {
...
// Ler arquivo Movel 2
document
  .getElementById("fileInputMovel2")
  .addEventListener("change", function (event) {
    var file = event.target.files[0];
...
function processMovelData2(content) {
  content.forEach((e) => {
    ...
    if (e.valor_unitario == 0 || e.valor_unitario == 0.0) {
      return;
    }
    ...
    let grupos = [
      { grupo: "Correção", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Input", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Pendente", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Antifraude", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Lançamento", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Pendências", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Qualidade", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Input MVP", fila: "AGUARDANDO_IMPUT_MVP" },
      { grupo: "Atualização", fila: "AGUARDANDO_IMPUT_MVP" },
      { grupo: "BKO Análise", fila: "ANALISE_BKO" },
      { grupo: "Em Tratativa", fila: "ANALISE_BKO" },
      { grupo: "Executado", fila: "FINALIZADO" },
      { grupo: "Cancelado", fila: "CANCELADO" },
      { grupo: "Correção", fila: "CORREÇÃO" },
      { grupo: "Em Tratativa", fila: "EM TRATATIVA" },
      { grupo: "Executado", fila: "FINALIZADO" },
      { grupo: "Fraude", fila: "MESA_FRAUDE" },
      { grupo: "A Executar", fila: "PENDENTE" },
      { grupo: "Cotação Reprovado", fila: "REPROVADO" },
      { grupo: "Aguardando Aceite", fila: "AGUARDANDO_IMPUT" },
      { grupo: "Cotação Análise", fila: "CREDITO" },
      { grupo: "BKO Reprovado", fila: "REPROVADO" },
      { grupo: "Pendente", fila: "AGUARDANDO_IMPUT" },
    ];
    ...
    let dataInit = parseInt(
      (e.data_venda.getTime() - dataBase) / 24 / 60 / 60 / 1000
    );
    e.data_venda = dataInit;
    let dataMov = parseInt(
      (e.DATA_MOVIMENTACAO.getTime() - dataBase2) / 24 / 60 / 60 / 1000
    );
...
function compareCdCnpjCliente(a, b) {
  if (String(a.CD_CNPJ_CLIENTE).length < String(b.CD_CNPJ_CLIENTE).length) {
    ...
    String(a.CD_CNPJ_CLIENTE).length > String(b.CD_CNPJ_CLIENTE).length
  ) {
    return 1;
  } else if (
    String(a.CD_CNPJ_CLIENTE).length == String(b.CD_CNPJ_CLIENTE).length
  ) {
    if (Number(a.CD_CNPJ_CLIENTE) < Number(b.CD_CNPJ_CLIENTE)) return -1;
    if (Number(a.CD_CNPJ_CLIENTE) > Number(b.CD_CNPJ_CLIENTE)) return 1;
  }
  return 0;
}
...
function compareTpSolicitacao(b, a) {
  if (a.TP_SOLICITACAO < b.TP_SOLICITACAO) return -1;
  if (a.TP_SOLICITACAO > b.TP_SOLICITACAO) return 1;
  return 0;
}
...
const orderFila = [
  ...
  "AGUARDANDO ESTOQUE",
  "AGUARDANDO_JANELA",
  "AGUARDANDO_PISTOLAGEM",
  "ANALISE_BKO",
  "CANCELADO",
  "CANCELADO_FRAUDE",
  "CREDITO",
  "CREDITO_NEGADO",
  "ERRO_SOLICITACAO_PORTIN",
  "EM_TRAMITE",
  "ERRO_ATIVACAO",
  "AGUARDANDO_IMPUT",
  "AGUARDANDO_IMPUT_MVP",
  "MESA_FRAUDE",
  "PENDENTE",
  "REPROVADO_BKO",
  "REPROVADO_CRED",
...
function teste() {
  dataFinal.forEach((e, index) => {
    if (
      e.TP_SOLICITACAO == "Alta em linha móvel" &&
      e.DS_STATUS_PEDIDO != "Executado"
    ) {
...
document.getElementById("exportBtn").addEventListener("click", function () {
  ...
  const wbout = XLSX.writeFile(workbook, "MovelFDV.xlsx", {
    compression: true,
  });
